...
export function createAirport(scene){
  ...
  const material=(name,color,{emissive=false,alpha=1}={})=>{if(materials.has(name))return materials.get(name);const value=BABYLON.Color3.FromHexString(PALETTE[color]||color);const result=new BABYLON.StandardMaterial(name,scene);result.diffuseColor=value;result.ambientColor=value.scale(.22);result.specularColor=new BABYLON.Color3(.06,.06,.06);result.specularPower=24;result.maxSimultaneousLights=6;result.alpha=alpha;if(emissive)result.emissiveColor=value.scale(.82);materials.set(name,result);return result};
  ...
  // These four fixtures are the real mobile-friendly lights. Their small visible
  // pendants make the source position obvious; the other ceiling strips remain
  // emissive-only decoration.
  ...
  const practicalLights=lightSpecs.map(([name,[x,y,z],intensity,range],index)=>{box(`${name} pendant stem`,[x,y+.42,z],[.12,.7,.12],'dark',false);box(`${name} pendant shade`,[x,y+.12,z],[1.05,.18,.72],'dark',false);const diffuser=box(`${name} visible diffuser`,[x,y,z],[.82,.1,.55],'white',false);diffuser.material=material('warm lamp diffuser',PALETTE.white,{emissive:true});const light=new BABYLON.PointLight(name,new BABYLON.Vector3(x,y-.12,z),scene);light.diffuse=BABYLON.Color3.FromHexString('#ffd9a0');light.specular=BABYLON.Color3.FromHexString('#ffe7bd');light.intensity=intensity;light.range=range;light.radius=.65;light.metadata={fixture:diffuser.name,performanceTier:storedQuality==='low'?'low':'standard',index};return light});
