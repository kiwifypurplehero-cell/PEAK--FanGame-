...
export function createAirportLobby(scene){
  ...
  const diagnosticGroups=[];let activeGroup=null;
  const beginGroup=name=>{activeGroup={name,nodes:[],lights:[]};diagnosticGroups.push(activeGroup);console.info(`[FEANK][SCENE GROUP] ${name} - begin`)};
  const tag=node=>{if(activeGroup&&node){node.metadata={...(node.metadata||{}),diagnosticGroup:activeGroup.name};activeGroup.nodes.push(node)}return node};
  const endGroup=()=>{if(activeGroup)console.info(`[FEANK][SCENE GROUP] ${activeGroup.name} - ready`,{nodes:activeGroup.nodes.length,lights:activeGroup.lights.length})};
  const mats=new Map();
  const mat=(name,color,options={})=>{if(mats.has(name))return mats.get(name);const m=new BABYLON.StandardMaterial(name,scene),c=BABYLON.Color3.FromHexString(color);m.diffuseColor=c;m.ambientColor=c.scale(.45);m.emissiveColor=c.scale(options.emissive?.45:.035);m.specularColor=options.glass?new BABYLON.Color3(.18,.22,.23):new BABYLON.Color3(.055,.055,.055);if(options.glass){m.alpha=isMobile?.55:.34;m.backFaceCulling=isMobile;m.needDepthPrePass=!isMobile}mats.set(name,m);return m};
  const box=(name,p,s,material,opts={})=>{const m=tag(BABYLON.MeshBuilder.CreateBox(name,{width:s[0],height:s[1],depth:s[2]},scene));m.position.set(...p);m.material=material;m.checkCollisions=opts.collisions??true;m.isPickable=opts.pickable??m.checkCollisions;return m};
  const cylinder=(name,p,height,diameter,material,opts={})=>{const m=tag(BABYLON.MeshBuilder.CreateCylinder(name,{height,diameter,tessellation:opts.tessellation||8},scene));m.position.set(...p);m.material=material;m.checkCollisions=opts.collisions??false;m.isPickable=false;return m};
  ...
  const sky=tag(BABYLON.MeshBuilder.CreateSphere('permanent sunrise sky',{diameter:78,segments:12,sideOrientation:BABYLON.Mesh.BACKSIDE},scene));sky.material=mat('sunrise sky','#c8b9e8',{emissive:true});sky.isPickable=false;sky.infiniteDistance=true;
  ...
  endGroup();beginGroup('GLASS');
  const glassMat=mat('blue sunrise glass',C.glass,{glass:true});
  for(let z=-5.5;z<=7;z+=3.1){const pane=box('skylight panel',[0,7.72,z],[5.8,.08,2.85],glassMat,{collisions:false,pickable:false});pane.visibility=isMobile?1:.88;box('skylight cross beam',[0,7.62,z-1.5],[6.2,.18,.14],mat('steel',C.beam),{collisions:false,pickable:false})}
  ...
  for(let x=-9.4;x<=9.4;x+=3.75){const pane=box('facade glass panel',[x,3.75,-7.05],[3.55,7.15,.09],glassMat,{collisions:true,pickable:true});pane.visibility=isMobile?1:.9;box('facade mullion',[x-1.84,3.75,-6.96],[.15,7.4,.2],mat('steel',C.beam))}
  ...
  const sun=tag(BABYLON.MeshBuilder.CreateDisc('low sunrise sun',{radius:1.45,tessellation:24},scene));sun.position.set(-8,4,-31);sun.material=mat('sun glow','#ffd37a',{emissive:true});sun.isPickable=false;
  box('distant terminal silhouette',[5,1,-28],[13,2,1],mat('distant lilac','#7b7894'),{collisions:false,pickable:false});

  endGroup();beginGroup('STRUCTURE DETAILS');
  ...
  endGroup();beginGroup('WORKBENCH/LAPTOP');
  const counter=tag(new BABYLON.TransformNode('Expedition Counter',scene));counter.position.set(-5.8,0,2.5);
  ...
  const laptop=tag(new BABYLON.TransformNode('Laptop',scene));laptop.parent=counter;laptop.position.set(0,0,0);
  ...
  endGroup();beginGroup('DECORATIONS');
  // An unmistakable in-world marker makes accidental test-platform regressions visible.
  const signMaterial=new BABYLON.StandardMaterial('airport lobby sign',scene);signMaterial.diffuseColor=BABYLON.Color3.FromHexString('#fff2c4');signMaterial.emissiveColor=new BABYLON.Color3(.35,.31,.22);signMaterial.specularColor=BABYLON.Color3.Black();
  if(!isMobile)try{const signTexture=new BABYLON.DynamicTexture('airport lobby sign text',{width:1024,height:192},scene,false);const signContext=signTexture.getContext();signContext.fillStyle='#fff2c4';signContext.fillRect(0,0,1024,192);signContext.fillStyle='#263940';signContext.font='bold 72px Arial';signContext.textAlign='center';signContext.textBaseline='middle';signContext.fillText('FEANK AIRPORT LOBBY',512,96);signTexture.update();signMaterial.diffuseTexture=signTexture}catch(error){console.warn('[FEANK] Decorative airport sign texture unavailable; using its plain material.',error)}
  const lobbySign=box('FEANK AIRPORT LOBBY sign',[0,6.15,8.42],[7.6,1.35,.1],signMaterial,{collisions:false,pickable:false});

  const plant=(x,z,scale=1)=>{cylinder('plant pot',[x,.42*scale,z],.84*scale,.7*scale,mat('terracotta',C.pot));const stem=cylinder('plant stem',[x,.98*scale,z],.55*scale,.09*scale,mat('stem','#406a4b'));for(let i=0;i<5;i++){const leaf=tag(BABYLON.MeshBuilder.CreateSphere('low poly leaf',{diameter:.65*scale,segments:5},scene));leaf.position.set(x+Math.sin(i*2.4)*.3*scale,1.22*scale+(i%2)*.22*scale,z+Math.cos(i*2.4)*.3*scale);leaf.scaling.y=1.45;leaf.material=mat('foliage',C.leaf);leaf.isPickable=false} };
  ...
  const kiosk=tag(new BABYLON.TransformNode('Kiosk',scene));kiosk.position.set(7.7,0,-.8);box('kiosk pedestal',[7.7,.85,-.8],[1.25,1.7,.75],mat('kiosk shell',C.dark));const kioskScreen=box('kiosk screen',[7.7,1.27,-1.19],[.82,.7,.035],mat('kiosk display','#87d6df',{emissive:true}),{collisions:false,pickable:true});
  ...
  endGroup();beginGroup('LIGHTING');
  const hemi=new BABYLON.HemisphericLight('soft terminal fill',new BABYLON.Vector3(0,1,0),scene);activeGroup.lights.push(hemi);hemi.diffuse=BABYLON.Color3.FromHexString('#dcecff');hemi.groundColor=BABYLON.Color3.FromHexString('#766f83');hemi.intensity=.72;
  ...
  activeGroup.lights.push(sunrise,fill);
  let quality='';try{quality=localStorage.getItem('feank.settings')||''}catch(error){console.warn('[FEANK] Saved quality is unavailable.',error)}
  if(!isMobile&&!quality.includes('"quality":"low"'))try{const shadows=new BABYLON.ShadowGenerator(512,sunrise);shadows.useBlurExponentialShadowMap=true;shadows.blurKernel=8;[counter,...scene.meshes.filter(m=>m.name.includes('pillar'))].forEach(m=>shadows.addShadowCaster(m));floor.receiveShadows=true}catch(error){floor.receiveShadows=false;console.warn('[FEANK] Optional terminal shadows unavailable; continuing without them.',error)}
  if(isMobile)scene.meshes.forEach(mesh=>{mesh.receiveShadows=false});
  endGroup();
  console.info('[FEANK] Mobile-safe airport graphics',{isMobile,shadows:!isMobile&&!quality.includes('"quality":"low"'),dynamicSignTexture:!isMobile,glassDepthPrePass:!isMobile});
  return {floor,laptop,kiosk:kioskScreen,lobbySign,diagnosticGroups,spawn:{position:{x:0,y:1.72,z:-4.5},target:{x:-4.8,y:1.5,z:2.5}},laptopPoint:new BABYLON.Vector3(-5.8,1.9,2.72),kioskPoint:new BABYLON.Vector3(7.7,1.3,-1.2)};
}
