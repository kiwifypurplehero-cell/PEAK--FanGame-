...
export class LobbyGame {
  ...
  async init(){
    const engine=this.core.create(),scene=new BABYLON.Scene(engine);scene.clearColor=BABYLON.Color4.FromHexString('#927fa8ff');scene.collisionsEnabled=true;scene.gravity=new BABYLON.Vector3(0,-.22,0);scene.imageProcessingConfiguration.exposure=1.08;scene.imageProcessingConfiguration.contrast=1.08;scene.imageProcessingConfiguration.toneMappingEnabled=true;scene.imageProcessingConfiguration.toneMappingType=BABYLON.ImageProcessingConfiguration.TONEMAPPING_ACES;scene.skipPointerMovePicking=true;
    const airport=createAirport(scene);this.player=new PlayerController(scene,this.canvas);if(airport.shadowGenerator)this.player.handsRoot.getChildMeshes().forEach(mesh=>airport.shadowGenerator.addShadowCaster(mesh));this.interactions=new InteractionManager(scene,this.player.camera,document.querySelector('#interaction-prompt'));this.ui=new LobbyPanelUI(open=>{this.player.setEnabled(!open);this.interactions.enabled=!open;if(open)this.interactions.select(null);this.onPanelChange(open)});
    ...
    await scene.whenReadyAsync();engine.resize();scene.render();console.info('[FEANK] Airport lobby ready',{camera:this.player.camera.position.asArray(),spawnSafe:true,meshes:scene.meshes.length,lights:scene.lights.length,lighting:'fixed sunrise',shadowMap:airport.shadowGenerator?.getShadowMap()?.getSize()||'disabled'});this.core.run(scene);
  }
