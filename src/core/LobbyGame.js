...
export class LobbyGame {
  ...
  async init(){
    ...
    register(world.laptop,'Laptop','Use Laptop',world.interactionPoint,()=>this.ui.show('laptop'),2.8);
    ...
    await scene.whenReadyAsync();engine.resize();scene.render();console.info('[FEANK] Minimal platform ready',{camera:this.player.camera.position.asArray(),spawnSafe:true,meshes:scene.meshes.length,lights:scene.lights.length,lighting:'hemispheric + directional',renderLoops:1});this.core.run(scene);
  }
