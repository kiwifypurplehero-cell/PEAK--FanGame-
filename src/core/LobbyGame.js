...
export class LobbyGame {
  ...
  async init(){
    ...
    await scene.whenReadyAsync();engine.resize();scene.render();console.info('[FEANK] Airport lobby ready',{camera:this.player.camera.position.asArray(),spawnSafe:true,meshes:scene.meshes.length,lights:scene.lights.length,lighting:'sunrise + fill + practical',renderLoops:1});this.core.run(scene);
  }
