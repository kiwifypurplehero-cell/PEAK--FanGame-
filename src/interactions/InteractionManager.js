...
export class InteractionManager {
  ...
  update(){if(!this.enabled)return this.select(null);const maxDistance=this.items.reduce((value,item)=>Math.max(value,item.interactionDistance),0);const ray=this.camera.getForwardRay(maxDistance);const hit=this.scene.pickWithRay(ray,m=>m.isPickable&&m.isEnabled());let node=hit?.pickedMesh,item=null;while(node&&!item){item=node.metadata?.interactable||null;node=node.parent}this.select(item&&hit.distance<=item.interactionDistance?item:null)}
  ...
  interact(){const item=this.current;if(item)item.interactionAction?.(item)}
