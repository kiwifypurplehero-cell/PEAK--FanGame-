...
export class InteractionManager {
  ...
  update(){if(!this.enabled)return this.select(null);const ray=this.camera.getForwardRay(3.5);const hit=this.scene.pickWithRay(ray,m=>m.isPickable!==false);const item=hit?.pickedMesh?.metadata?.interactable||null;this.select(item&&hit.distance<=item.interactionDistance?item:null)}
