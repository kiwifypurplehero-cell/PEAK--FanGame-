export class MenuController {
  ...
  init(){
    document.querySelectorAll('[data-panel]').forEach(button=>{button.addEventListener('click',()=>this.open(button.dataset.panel,button));button.addEventListener('pointerenter',()=>this.audio.play('hover'))});
  ...
  close(){if(!this.active)return;this.audio.play('close');this.active.classList.remove('active');this.active=null;this.overlay.classList.remove('open');this.overlay.setAttribute('aria-hidden','true');this.scene.classList.remove('obscured');this.scene.setAttribute('aria-hidden','false');this.returnFocus?.focus()}
